...
import { TopicManager, SubscribeHandler, Subscribe } from "./MQTTContext";

export class Historing implements TopicManager {
    handler: number;
    ...
    subscribe(subscription: SubscribeHandler, subscribe: Subscribe): boolean {
        if (subscription.topic.startsWith("HISTORY://")) {
            const h = subscribe("fsfsf", () => {});
            if (h) {
